...
describe('Infix Expressions', () => {
    let lhs = new cgl.Reference(
        new cgl.InterfaceVariable(cgl.Qualifier.In, new cgl.Variable(new cgl.Type(cgl.Kind.Int), '1'))
    );
    let rhs = new cgl.Reference(
        new cgl.InterfaceVariable(cgl.Qualifier.In, new cgl.Variable(new cgl.Type(cgl.Kind.Float), '1.5'))
    );

    describe('source', () => {
        it ('addition', () => {
            const operation = new cgl.Addition(lhs, rhs);
            expect(operation.source()).to.equalIgnoreSpaces('1 + 1.5');
        });
    ...
    describe('returnType', () => {
        ...
        it('returns int when both of the variables are of type int', () => {
            const operation = new cgl.Subtraction(lhs, otherIntType);
    ...
    describe('errors', () => {
        let stringVar = new cgl.Reference(
            new cgl.InterfaceVariable(cgl.Qualifier.In, new cgl.Variable(new cgl.Type(cgl.Kind.String), 'rhs'))
        );
